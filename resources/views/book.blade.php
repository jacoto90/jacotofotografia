    ...
    <form id="downloadForm" method="POST" action="{{ route('book.download') }}" class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="relative bg-white rounded-lg shadow-md overflow-hidden group">
                <img src="{{ asset($image) }}" alt="Foto" class="w-full h-auto" loading="lazy">
                <div class="checkbox-container absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
...
<style>.checkbox-container { opacity: 0; } .group:hover .checkbox-container { opacity: 1; } .checkbox-container.checked { opacity: 1 !important; } .checkbox-container:has(input:checked) { opacity: 1 !important; }</style>
<script>
    let scrollInterval;
    const speed = 1;
    ...
    function syncCheckboxes() {
        document.querySelectorAll('input[name="selectedImages\\[\\]"]').forEach(function(cb) {
            cb.closest('.checkbox-container').classList.toggle('checked', cb.checked);
        });
    }
    document.querySelectorAll('input[name="selectedImages\\[\\]"]').forEach(function(cb) {
    ...
    function selectAll() {
        document.querySelectorAll('input[name="selectedImages\\[\\]"]').forEach(function(cb) { cb.checked = true; syncCheckboxes(); });
    }

    function deselectAll() {
        document.querySelectorAll('input[name="selectedImages\\[\\]"]').forEach(function(cb) { cb.checked = false; syncCheckboxes(); });
    }
